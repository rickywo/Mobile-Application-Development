...
/**
 * Created by Ricky Wu on 2015/8/19.
 */
public class MovieEvent {
    // General information from IMDB
    private String _title;
    private String _year;
    private String _shorPlot;
    ...
    private Calendar _partyDate;
    private String _partyVenue;
    private String _location;
    private ArrayList<String> _mailList;

    public MovieEvent(String jsonString, int imageId) {
        Map<String, String> map = new Gson().fromJson(jsonString, new TypeToken<Map<String, String>>() {
        }.getType());
        _title = map.get("Title");
        _year = map.get("Year");
        _shorPlot = map.get("Plot");
        _longPlot = "";
        _imdbId = map.get("imdbId");
        _imdbRating = map.get("imdbRating");
        _genre = map.get("Genre");
        _img = imageId;
        _rating = 0.0f;
        _partyDate = Calendar.getInstance();
        _partyVenue = "";
        _location = "";
    }
    ...
    public void setLongPlot(String plot) {
        _longPlot = plot;
    }

    public void addInvitee(String invitee) {
        _mailList.add(invitee);
    }

    public void removeInvitee(String invitee) {
        _mailList.remove(invitee);
    }
     ...
     * Created by Ricky Wu on 2015/8/19.
     */
    public static class MovieEventModel {
        private ArrayList<MovieEvent> movieEvents = new ArrayList<MovieEvent>();
        static final String[] mvJson = {
        ...
        public MovieEventModel() {
            for(int i = 0 ;i < image.length ; i++) {
                MovieEvent m = new MovieEvent(mvJson[i], image[i]);
                movieEvents.add(m);
            }
        }

        public ArrayList<MovieEvent> getEvents() {
            return movieEvents;
        }
    }
}
